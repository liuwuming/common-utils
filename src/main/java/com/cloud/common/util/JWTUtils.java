...
public class JWTUtils {
   ...
   static {
	...
	public static String getToken(String uid, int exp) {
		long endTime = System.currentTimeMillis() + 1000L * 60 * exp;
		return Jwts.builder().setId(uid).setSubject(uid).setExpiration(new Date(endTime))
	...
	public static String getToken(String uid) {
		long endTime = System.currentTimeMillis() + 1000L * 60 * 1440 * 30;
		return Jwts.builder().setId(uid).setSubject(uid).setExpiration(new Date(endTime))
				.signWith(SignatureAlgorithm.RS512, priKey).compact();
	}
	 ...
	 * @return JWTResult
	 */
	public JWTResult checkToken(String token,String uid) {
		try {
			Claims claims = Jwts.parser().setSigningKey(pubKey).parseClaimsJws(token).getBody();
			if(null==claims || ! claims.getId().equals(uid)) {
